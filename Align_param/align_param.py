...
- il y a au moins une tabulation réelle entre le type et le nom,
...
def make_param_separator(type_start_col: int, type_text: str, target_name_col: int, tab_size: int) -> str:
    """
    Construit un séparateur contenant au moins une tabulation réelle,
    puis éventuellement des espaces pour atteindre target_name_col.
    """
    type_end_col = visual_width(type_text, type_start_col, tab_size)
    after_tab_col = next_tab_stop(type_end_col, tab_size)
    spaces = max(0, target_name_col - after_tab_col)
    return "\t" + (" " * spaces)
