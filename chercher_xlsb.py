            ...
            try:
                ...
                for nom_mode, look_in in obtenir_modes_find(search_in):
                    ...
                    while cellule is not None:
                        ...
                        # Important : avec Excel, Find(..., LookIn=xlFormulas) peut aussi
                        # remonter des cellules texte/constantes, car leur propriété Formula
                        # reflète parfois le contenu saisi. Pour --search-in formulas, on veut
                        # uniquement les vraies cellules contenant une formule Excel.
                        if nom_mode == "formule" and not has_formula:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="both",
        help=(
            "Cherche dans les vraies formules Excel, dans les valeurs affichées/calculées, "
            "ou dans les deux (par défaut: both)."
        ),
    )
    parser.add_argument(
        "--engine",
