...
def obtenir_ligne_colonne_depuis_range(cellule) -> Tuple[int, int]:
    """
    Récupère la ligne et la colonne d'un objet Range Excel COM de manière robuste.

    On évite volontairement `cellule.Address(...)` : selon la version de pywin32/
    dispatch utilisée, `Address` peut parfois être renvoyé comme une simple chaîne,
    ce qui provoque l'erreur `'str' object is not callable` quand on essaie de
...
def obtenir_adresse_depuis_range(cellule) -> str:
    ligne, colonne = obtenir_ligne_colonne_depuis_range(cellule)
    ...
    try:
        ...
        for feuille in classeur.Worksheets:
            ...
            try:
                ...
                cellule_depart = plage.Cells.Item(plage.Cells.Count)
                ...
                for nom_mode, look_in in obtenir_modes_find(search_in):
                    ...
                    premiere_adresse = obtenir_adresse_depuis_range(premiere)
                    cellule = premiere

                    while cellule is not None:
                        adresse = obtenir_adresse_depuis_range(cellule)
                        ...
                        adresse_suivante = obtenir_adresse_depuis_range(cellule)
